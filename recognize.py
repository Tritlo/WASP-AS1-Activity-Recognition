...
def readData(filename):
    with open(filename) as f:
        ...
        for line in f.readlines():
            ln = line.split()
            if ln[1] == "ACC":
                acc.append(list(map(float, ln[2:])))
...
def buildClassifier(runningData, walkingData, standingData,
                    WINDOW=100, STEPS=25):
    data = [runningData, walkingData, standingData]
    windows = list(map(lambda d: calcSlidingWindowVals(d, WINDOW, STEPS), data))
...
def classify(classifier, data):
    ...
    def classifyWindow(subd):
        ...
        (classification, _) = min(dists, key=lambda m: m[1])
    ...
    if i+2 < len(data):
        ...
        for j in range(i, len(data)):
            classifiedData.append((classifiedData[-1][0], classifiedData[-1][1], data[j]))
...
def plotClassifiedData(data, filename="Classification.png"):
    ...
    f = plt.fill_between(range(len(labels)), maxY, minY,
                         where=([la == 'standing' for la in labels]),
                         color='Blue', alpha=0.3)
    f.set_label('Standing')
    f = plt.fill_between(range(len(labels)), maxY, minY,
                         where=([la == 'walking' for la in labels]),
                         color='Green', alpha=0.3)
    f.set_label('Walking')
    f = plt.fill_between(range(len(labels)), maxY, minY,
                         where=([la == 'running' for la in labels]),
                         color='Red', alpha=0.3)
    f.set_label('Running')
...
def plotClassifier(classifier, filename):
    ...
    ln = plt.axhline(db[-1][1], color='Red', linestyle='dotted')
    ln.set_label(db[-1][0])
    plt.axhspan(db[-1][1], maxY*1.1, color='Red', alpha=alpha)

    ln = plt.axhline(db[0][1], color='Green', linestyle='dotted')
    ln.set_label(db[0][0])
